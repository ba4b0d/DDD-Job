...
export default function Categories() {
  ...
  // Build parent options (exclude current category to prevent loops)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Create/Edit modal — shared Modal component */}
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editing ? 'ویرایش دسته‌بندی' : 'دسته‌بندی جدید'} size="md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>نام دسته‌بندی</label>
            <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
              className="input-field" required autoFocus />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>توضیحات (اختیاری)</label>
            <input type="text" value={form.description} onChange={e => setForm({...form, description: e.target.value})}
              className="input-field" />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>دسته‌بندی والد (اختیاری)</label>
